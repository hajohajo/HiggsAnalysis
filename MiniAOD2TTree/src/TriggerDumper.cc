    ...
    //  trgL1ETMToken(iConsumesCollector.consumes<std::vector<l1extra::L1EtMissParticle>>(pset.getParameter<edm::InputTag>("L1Extra"))) 
{
  ...
  if (0) std::cout << "TriggerDumper::TriggerDumper() L1TauObjects" << std::endl;
  if(inputCollection.exists("L1TauObjects")){
    l1TausToken = iConsumesCollector.consumes<l1t::TauBxCollection>(pset.getParameter<edm::InputTag>("L1TauObjects"));
    bookL1Tau = true;
  }

  if (0) std::cout << "TriggerDumper::TriggerDumper() L1JetObjects" << std::endl;
  ...
  if (0) std::cout << "TriggerDumper::TriggerDumper() L1EtSumObjects" << std::endl;
  if(inputCollection.exists("L1EtSumObjects")){
    l1EtSumToken = iConsumesCollector.consumes<l1t::EtSumBxCollection>(pset.getParameter<edm::InputTag>("L1EtSumObjects"));
    bookL1EtSum = true;
  }

  if (0) std::cout << "TriggerDumper::TriggerDumper() TriggerPrescales" << std::endl;
...
void TriggerDumper::book(const edm::Run& iRun, HLTConfigProvider hltConfig){
  ...
  if (0) std::cout << "TriggerDumper::TriggerDumper() book" << std::endl;
...
bool TriggerDumper::fill(edm::Event& iEvent, const edm::EventSetup& iSetup){
  if (0) std::cout << "TriggerDumper::fill() " << std::endl;
  /*
  ...
  */
  ...
  if (0) std::cout << "TriggerDumper::fill() trgResultsHandle" << std::endl;
  if(trgResultsHandle.isValid())
    ...
    if (0) std::cout << "TriggerDumper::fill() patTriggerObjec" << std::endl;
	...
	if(patTriggerObject.id(trigger::TriggerMET))
	  {
	    if (0) std::cout << "TriggerDumper::fill() TriggerMET" << std::endl;
	...
	*/
	...
	if(patTriggerObject.id(trigger::TriggerTau))
	  {
	    if (0) std::cout << "TriggerDumper::fill() TriggerTau" << std::endl;
	    bool fired = false;
	...
	// Printouts disabled
	  ...
	  {
	    if (0) std::cout << "TriggerDumper::fill() TriggerElectron" << std::endl;
	    bool fired = false;
	...
	if(patTriggerObject.id(trigger::TriggerBJet))
	  {
	    if (0) std::cout << "TriggerDumper::fill() TriggerBJet" << std::endl;
	    bool fired = false;
	    for(size_t i = 0; i < trgMatchPaths.size(); ++i){
    ...
    if (0) std::cout << "TriggerDumper::fill() trgPrescalePaths" << std::endl;   
	...
	if(trgPrescaleHandle.isValid())
	  {
	    ...
	    for(std::vector<std::string>::const_iterator i = trgPrescalePaths.begin(); i != trgPrescalePaths.end(); ++i)
	      {
		if (0) std::cout << "TriggerDumper::fill() prescale name = " << *i << std::endl;
  ...
  if (0) std::cout << "TriggerDumper::fill() filter" << std::endl;   
  return filter();
}
