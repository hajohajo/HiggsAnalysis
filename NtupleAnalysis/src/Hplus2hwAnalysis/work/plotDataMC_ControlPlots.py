...
EXAMPLES:
./plotDataMC_ControlPlots.py --folder ForDataDrivenCtrlPlots -e "ZJetsToQQ_HT600toInf" --ratio -s png -m
./plotDataMC_ControlPlots.py --folder PUDependency -e "ZJetsToQQ_HT600toInf" --ratio -s png -m 
./plotDataMC_ControlPlots.py --folder counters/weighted -e "ZJetsToQQ_HT600toInf" --ratio -s png -m 
./plotDataMC_ControlPlots.py --folder muSelection_ -e "ZJetsToQQ_HT600toInf" --ratio -s png -m
./plotDataMC_ControlPlots.py --folder tauSelection_ -e "ZJetsToQQ_HT600toInf" --ratio -s png -m
./plotDataMC_ControlPlots.py --folder jetSelection_ -e "ZJetsToQQ_HT600toInf" --ratio -s png -m


LAST USED: (after removing WJets (non-HT) and DYJets (non-HT) from pseudomulticrab multicrab.cfg)
./plotDataMC_ControlPlots.py --folder ForDataDrivenCtrlPlots -e "ZJetsToQQ_HT600toInf" --ratio -s png -m
./plotDataMC_ControlPlots.py --folder PUDependency -e "ZJetsToQQ_HT600toInf" --ratio -s png -m 
./plotDataMC_ControlPlots.py --folder counters/weighted -e "ZJetsToQQ_HT600toInf" --ratio -s png -m 
./plotDataMC_ControlPlots.py --folder muSelection_ -e "ZJetsToQQ_HT600toInf" --ratio -s png -m
...
def main(opts):
    ...
    for opt in optModes:
        ...
        for i, h in enumerate(myHistos, 1):
            ...
            if "topEff_Vs_TopPt_Inclusive" in h or "topEff_Vs_TopPt_Fake" in h:
                continue
...
def GetHistoKwargs(h, opts):
    ...
    myBins  = []
    ptBins  = []
    _legNE  = {"dx": -0.06, "dy": -0.01, "dh": 0.0}
    _legNW  = {"dx": -0.52, "dy": -0.01, "dh": 0.0}
    _legSE  = {"dx": -0.06, "dy": -0.50, "dh": 0.0}
    _legSW  = {"dx": -0.52, "dy": -0.50, "dh": 0.0}
    _legCE  = {"dx": -0.08, "dy": -0.44, "dh": 0.0}
    ...
    if "BDT" in h:
        ...
        kwargs["opts"]   = {"xmin": -1.0, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["rebinX"] = 2        
    ...
    if "RelIsoBefore" in h:
        ...
        kwargs["opts"]   = {"xmin": 0.0, "xmax": +60.0, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["rebinX"] = 5

    if "RelIsoAfter" in h or "RelIsoPassed" in h:
        kwargs["ylabel"] = "Events / %.0f "
        kwargs["opts"]   = {"xmin": 0.0, "xmax": +20.0, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["rebinX"] = 5

    if "RelIsoAll" in h:
        kwargs["ylabel"] = "Events / %.0f "
        kwargs["opts"]   = {"xmin": 0.0, "xmax": +200.0, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["rebinX"] = 10
    ...
    if "jetpt" in h.lower():
        ...
        kwargs["xlabel"] = "p_{T} (%s)" % units
        kwargs["ylabel"] = _yLabel + units
        kwargs["opts"]    = {"xmin": 0.0, "xmax": 700.0, "ymin": 1e-2, "ymaxfactor": yMaxF}
    ...
    if h == "tauNpassed":
        ...
        kwargs["cutBox"] = {"cutValue": 2.0, "fillColor": 16, "box": False, "line": True, "greaterThan": False}

    if "HT" in h or "JT" in h:
        ROOT.gStyle.SetNdivisions(8, "X")
        kwargs["rebinX"] = 2 #systematics._dataDrivenCtrlPlotBinning["HT_AfterAllSelections"]
        units            = "GeV"
        kwargs["opts"]   = {"xmin": 0.0, "xmax": +1000.0, "ymin": yMin, "ymaxfactor": yMaxF}
    ...
    if(regex.search(h)):
        ...
        if "fifth" in h.lower():
            kwargs["opts"] = {"xmin": 0.0, "xmax": +300.0, "ymin": yMin, "ymaxfactor": yMaxF}
        if "sixth" in h.lower():
            kwargs["opts"] = {"xmin": 0.0, "xmax": +100.0, "ymin": yMin, "ymaxfactor": yMaxF}
    ...
    if "MHT" in h:
        kwargs["rebinX"] = 2 #systematics._dataDrivenCtrlPlotBinning["MET_AfterAllSelections"] #2
        units            = "GeV"
    ...
    if "NBjets" in h:
        ...
        kwargs["opts"]   = {"xmin": 0.0, "xmax": 8.0, "ymin": yMin, "ymaxfactor": yMaxF}
        kwargs["cutBox"] = {"cutValue": 1, "fillColor": 16, "box": False, "line": True, "greaterThan": True}
