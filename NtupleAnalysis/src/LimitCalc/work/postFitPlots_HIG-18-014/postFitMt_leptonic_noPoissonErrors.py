...
# This one does not have Poisson errors implemented, so it should be only used for plots with high event counts in all bins,
# i.e. the leptonic post-fit plots.
#
# Usage:
    ...
    def fetchHistograms(self,fINdata,fINsignal,fINpost):
        ...
        for iBin in range(0,self.h_data.GetNbinsX()+1):
            if self.h_data.GetBinContent(iBin) < 0.00000001:
                self.h_data.SetBinContent(iBin,0.0)
                self.h_data.SetBinError(iBin,1.0)
...
#        self.h_data.SetMarkerSize(2)
        ...
        hhd = histograms.Histo(self.h_data,"Data",legendStyle=None, drawStyle="EP")
...
#        myParams["ylabel"] = "< Events / GeV >" # if divideByBinWidth = True
        ...
        plots.drawPlot(p,os.path.join(plotDir, name+"_preliminary"),**myParams)
