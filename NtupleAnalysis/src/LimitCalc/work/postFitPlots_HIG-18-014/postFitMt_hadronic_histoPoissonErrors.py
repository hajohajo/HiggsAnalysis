...
#This one has the Poisson errors implemented according to https://twiki.cern.ch/twiki/bin/viewauth/CMS/PoissonErrorBars
...
#        # Set bin errors to 1 for empty bins
        for iBin in range(0,self.h_data.GetNbinsX()+1):
            if self.h_data.GetBinContent(iBin) < 0.00000001:
                self.h_data.SetBinContent(iBin,0.00001)
                self.h_data.SetBinError(iBin,1.82)

        # Draw Poissonian errors
        self.h_data.GetSumw2().Set(0)
        self.h_data.SetBinErrorOption(ROOT.TH1.kPoisson)
...
#        self.h_data.SetMarkerSize(2)
        ...
        hhd = histograms.Histo(self.h_data,"Data",legendStyle=None, drawStyle="E0")
...
#        name = "preFitMt_"+self.name
        ...
        p.addMCUncertainty(postfit=True) #True)
...
#        myParams["ylabel"] = "< Events / GeV >" # if divideByBinWidth = True
        ...
        plots.drawPlot(p,os.path.join(plotDir, name+"_histoPoissonErrors"),**myParams)
        # Draw PRELIMINARY version
        histograms.cmsTextMode = histograms.CMSMode.PRELIMINARY
        plots.drawPlot(p,os.path.join(plotDir, name+"_preliminary"+"_histoPoissonErrors"),**myParams)
