...
OptionBr                               = 0.1   # [default: 1.0]    (The Br(t->bH+) used in figures and tables)
...
PlotLegendHeader                       = "H^{#pm}#rightarrowW^{#pm}H, H#rightarrow #tau^{+}_{h} #tau^{-}_{h}"
...
myLeptonVetoSystematics = ["CMS_eff_e_veto"]
myTauSystematics	= ["CMS_scale_t","CMS_eff_t","CMS_eff_t_highpt"]
myMuonSystematics	= ["CMS_eff_m"]
myJetSystematics        = ["CMS_scale_j", "CMS_res_j"]
myBtagSystematics       = ["CMS_eff_b"]
myVVSystematics         = ["CMS_scale_VV","CMS_pdf_VV"]

# Define systematics dictionary (easier access)
mySystematics = {}
mySystematics["QCD"]    = ["CMS_FakeRate"]
mySystematics["MC"]     = myLumiSystematics + myPileupSystematics + myTrgEffSystematics + myTauSystematics + myMuonSystematics + myLeptonVetoSystematics + myJetSystematics + myBtagSystematics + myTopTagSystematics + ["CMS_scale_met"]
...
mySystematics["VV"]     = mySystematics["MC"] + myVVSystematics
...
TES_Shape   = Nuisance(id="CMS_scale_t", label="tau Energy Scale",distr="shapeQ", function="ShapeVariation", systVariation="TauES")
tau_high_ID_Shape   = Nuisance(id="CMS_eff_t_highpt", label="tau ID",distr="shapeQ", function="ShapeVariation", systVariation="TauIDSyst")
trg_Shape   = Nuisance(id="CMS_eff_trg_MC", label="Trigger MC efficiency",distr="shapeQ", function="ShapeVariation", systVariation="ElectronTrgEffData")
muonID_Shape   = Nuisance(id="CMS_eff_m", label="mu ID",distr="shapeQ", function="ShapeVariation", systVariation="ElectronIDSyst")
met_Shape = Nuisance(id="CMS_scale_met", label="Unclustered MET energy scale", distr="shapeQ", function="ShapeVariation", systVariation="UES")
...
eVeto_Const     = Nuisance(id="CMS_eff_e_veto"     , label="e veto", distr="lnN", function="Constant", value=0.02)  #"Ratio", numerator="passed e selection (Veto)", denominator="Fake tau SF", scaling=0.02)
muVeto_Const    = Nuisance(id="CMS_eff_m_veto"     , label="mu veto (Approx.)" , distr="lnN", function="Constant", value=0.10)
tauVeto_Const   = Nuisance(id="CMS_eff_t"          , label="tau ID", distr="lnN", function="Constant", value=0.08)
...
vv_scale_Const = Nuisance(id="CMS_scale_VV", label="diboson cross section scale uncertainty",distr="lnN", function="Constant",value=systematics.getCrossSectionUncertainty("Diboson_scale").getUncertaintyDown())
vv_pdf_Const = Nuisance(id="CMS_pdf_VV", label="diboson pdf uncertainty",distr="lnN", function="Constant",value=systematics.getCrossSectionUncertainty("Diboson_pdf").getUncertaintyDown())
...
if OptionShapeSystematics:
    ...
    Nuisances.append(TES_Shape)
    Nuisances.append(trg_Shape)
    Nuisances.append(muonID_Shape)
    Nuisances.append(tau_high_ID_Shape)
    Nuisances.append(bTagSF_Shape)
    Nuisances.append(met_Shape)
...
Nuisances.append(vv_scale_Const)
Nuisances.append(vv_pdf_Const)
...
hElePt = ControlPlotInput(title="SelectedEle_pT_AfterAllSelections",
        histoName="SelectedEle_pT_AfterAllSelections",
        details={ "xlabel": "Selected e p_{T}",
...
hMuonPt = ControlPlotInput(title="SelectedMu_pT_AfterAllSelections",
        histoName="SelectedMu_pT_AfterAllSelections",
        details={ "xlabel": "Selected #mu p_{T}",
          "ylabel": "Events/^{}#Deltap_{T}",
          "divideByBinWidth": True,
          "unit": "GeV/c",
          "log": True,
...
#================================================================================================ 
ControlPlots.append(hElePt)
ControlPlots.append(hMuonPt)
