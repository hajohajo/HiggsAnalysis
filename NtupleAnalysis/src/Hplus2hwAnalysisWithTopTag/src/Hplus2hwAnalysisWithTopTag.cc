...
class Hplus2hwAnalysisWithTopTag: public BaseSelector {
public:
  explicit Hplus2hwAnalysisWithTopTag(const ParameterSet& config, const TH1* skimCounters);
  virtual ~Hplus2hwAnalysisWithTopTag() {}
...
REGISTER_SELECTOR(Hplus2hwAnalysisWithTopTag);

Hplus2hwAnalysisWithTopTag::Hplus2hwAnalysisWithTopTag(const ParameterSet& config, const TH1* skimCounters)
  : BaseSelector(config, skimCounters),
    fCommonPlots(config.getParameter<ParameterSet>("CommonPlots"), CommonPlots::kHplus2hwAnalysisWithTopTag, fHistoWrapper), // CommonPlots::kHplus2hwAnalysisWithTop
...
void Hplus2hwAnalysisWithTopTag::book(TDirectory *dir) {

  if (0) std::cout << "=== Hplus2hwAnalysisWithTopTag::book()" << std::endl;
...
void Hplus2hwAnalysisWithTopTag::setupBranches(BranchManager& branchManager) {
  fEvent.setupBranches(branchManager);
  return;
}


void Hplus2hwAnalysisWithTopTag::process(Long64_t entry) {
  ...
  if (1) std::cout << "=== Tau Selection: isGenuineTau = " << isGenuineTau << " (LdgTau = " << bGenuineLdgTau << ", SubldTau = " << bGenuineSubldgTau << ")" << std::endl;
