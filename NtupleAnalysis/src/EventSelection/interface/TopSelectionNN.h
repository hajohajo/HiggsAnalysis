// -*- c++ -*-
#ifndef EventSelection_TopSelectionNN_h
#define EventSelection_TopSelectionNN_h
...
class TopSelectionNN: public BaseSelection {
public:
  class Data {
  public:
    ...
    bool passedMVACut() const { return bPassedMVACut; } // for historic reasons (+ backgwards compatibility)
  ...
  explicit TopSelectionNN(const ParameterSet& config, EventCounter& eventCounter, HistoWrapper& histoWrapper, CommonPlots* commonPlots, const std::string& postfix = "");
  /// Constructor without histogramming
  explicit TopSelectionNN(const ParameterSet& config);
  virtual ~TopSelectionNN();
