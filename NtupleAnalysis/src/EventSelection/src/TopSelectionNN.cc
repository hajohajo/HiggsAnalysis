...
TopSelectionNN::TopSelectionNN(const ParameterSet& config, EventCounter& eventCounter, HistoWrapper& histoWrapper, CommonPlots* commonPlots, const std::string& postfix)
  : BaseSelection(eventCounter, histoWrapper, commonPlots, postfix),
    ...
    cPassedTopSelectionNN(fEventCounter.addCounter("passed top selection ("+postfix+")")),
    // Sub counters
...
TopSelectionNN::TopSelectionNN(const ParameterSet& config)
: BaseSelection(),
  // Input parameters
  cfg_NumberOfTopsCut(config, "NumberOfTopsCut"),
  cfg_TopMVACut(config     , "TopMVACut"),
  ...
  cfg_CSV_bDiscCut(config   , "CSV_bDiscCut"),
  // Event counter for passing selection
  cPassedTopSelectionNN(fEventCounter.addCounter("top")),
  // Sub counters
...
void TopSelectionNN::initialize(const ParameterSet& config) {
  ...
  std::cout << "Opening NN weight file " << fullPath << std::endl;
...
void TopSelectionNN::bookHistograms(TDirectory* dir) {
  // Fixed binning  
...
TopSelectionNN::Data TopSelectionNN::silentAnalyze(const Event& event, const JetSelection::Data& jetData, const BJetSelection::Data& bjetData) {
...
TopSelectionNN::Data TopSelectionNN::analyze(const Event& event, const JetSelection::Data& jetData, const BJetSelection::Data& bjetData) {
  ensureAnalyzeAllowed(event.eventID());

  // Ready to analyze
  TopSelectionNN::Data data = privateAnalyze(event, jetData.getSelectedJets(), bjetData.getSelectedBJets());

  // Send data to CommonPlots
  // if (fCommonPlots != nullptr) fCommonPlots->fillControlPlotsAtTopSelectionNN(event, data);// fixme - implement
  return data;
}


TopSelectionNN::Data TopSelectionNN::privateAnalyze(const Event& event, const std::vector<Jet> selectedJets, const std::vector<Jet> selectedBjets) {
  Data output;
  cSubAll.increment();
  ...
  // Top Candidates
  //================================================================================================  
  if (0) std::cout << "=== TopSelectionNN:: Obtaining all top candidates" << std::endl;
	  ...
	  for (auto& jet2: jets)
	    {
	      ...
	      //std::vector<std::string> inputNames = model.GetInputNames();
	      ...
	      if (0) std::cout<<"output = "<<MVAoutput<<endl;
	      ...
	      // Save top candidates
	      fAllTops.BDTG.push_back(MVAoutput);
	      ...
	      if (cfg_TopMVACut.passedCut(MVAoutput))
		{
		  ...
		  fSelectedTops.BDTG.push_back(MVAoutput);
	      ...
	      else
		{
		  ...
		  fNotSelectedTops.BDTG.push_back(MVAoutput);
  ...
  //================================================================================================    
  if (0) std::cout << "=== TopSelectionNN::Sort top candidates in MVA" << std::endl;
  ...
  for (size_t i = 0; i < fSelectedTops.BDTG.size(); i++)
    {
      ...
      hTopMVA_SelectedCandidates -> Fill(fSelectedTops.BDTG.at(i) );
      ...
      if (printCleanedTops)
	{
	  std::cout << i+1 << ") jet1 = " << fSelectedTops.Jet1.at(i).index() << ", jet2 = " << fSelectedTops.Jet2.at(i).index() 
		    << ", bjet = " << fSelectedTops.BJet.at(i).index() << ", MVA = " << fSelectedTops.BDTG.at(i)
      ...
      hTopMVA_SelectedCleanedCandidates -> Fill(fSelectedTops.BDTG.at(i) );
  ...
  for (size_t i = 0; i < fAllTops.BDTG.size(); i++)
	...
	std::cout << std::setw(12) << fAllTops.Jet1.at(i).index()      << std::setw(12)   << fAllTops.Jet2.at(i).index()   << std::setw(12)   << fAllTops.BJet.at(i).index()
      ...
      // Fill Histos (selected, cross-cleaned candidates)
      hTopMVA_AllCleanedCandidates -> Fill(fAllTops.BDTG.at(i) );
  ...
  for (size_t i = 0; i < fNotSelectedTops.BDTG.size(); i++)
    {
      // Fill Histos (selected candidates)
      hTopMVA_NotSelectedCandidates -> Fill(fNotSelectedTops.BDTG.at(i) );
  ...
  if (event.isMC())
	...
	{      
      ...
      if (0) output.fTopTaggingScaleFactorEventWeight = fTopTagSFCalculator.calculateSF(fAllCleanedTops.TrijetP4, fAllCleanedTops.BDTG, fAllCleanedTops.isTagged, fAllCleanedTops.isGenuine); //soti fixme
    }
  ...
  // Fill output data
  //================================================================================================
  if (0) std::cout << "=== TopSelectionNN:: Fill output" << std::endl;
  if (fAllCleanedTops.BDTG.size() > 0) 
    {
      
      output.bPassedMVACut = cfg_TopMVACut.passedCut( fAllCleanedTops.BDTG.at(0) );
      // Leading-in-MVA top
      output.fTopMVA      = fAllCleanedTops.BDTG.at(0);
  ...
  for (size_t i = 0; i < fSelectedTops.BDTG.size(); i++)
    {
      ...
      output.fSelectedTopsMVA.push_back(fSelectedTops.BDTG.at(i));
    }
  ...
  for (size_t i = 0; i < fNotSelectedTops.BDTG.size(); i++)
    {
      ...
      output.fNotSelectedTopsMVA.push_back(fNotSelectedTops.BDTG.at(i));
    }
  ...
  for (size_t i = 0; i < fAllTops.BDTG.size(); i++)
    {
      ...
      output.fAllTopsMVA.push_back(fAllTops.BDTG.at(i));
  ...
  for (size_t i = 0; i < fSelectedCleanedTops.BDTG.size(); i++)
    {
      ...
      output.fSelectedCleanedTopsMVA.push_back(fSelectedCleanedTops.BDTG.at(i));
    }
  ...
  for (size_t i = 0; i < fAllCleanedTops.BDTG.size(); i++)
    {
      ...
      output.fAllCleanedTopsMVA.push_back(fAllCleanedTops.BDTG.at(i));
  ...
  // Fill histograms
  //================================================================================================
  if (0) std::cout << "=== TopSelectionNN:: Filling histograms" << std::endl;
  if (output.bPassedSelection) cPassedTopSelectionNN.increment();
...
bool TopSelectionNN::areSameJets(const Jet& jet1, const Jet& jet2) {
...
bool TopSelectionNN::isBJet(const Jet& jet, const std::vector<Jet>& bjets) {
  for (auto bjet: bjets)
    {
...
Jet TopSelectionNN::getLeadingSubleadingJet(const Jet& jet0, const Jet& jet1, string selectedJet){
...
genParticle TopSelectionNN::getLeadingSubleadingParton(const genParticle& quark0, const genParticle& quark1, string selectedParton){
...
bool TopSelectionNN::isMatchedJet(const Jet& jet, const TrijetSelection& myTops, const unsigned int index) {
...
vector<float> TopSelectionNN::GetListOfInputs(std::vector<std::string> inputNames, const Jet& jet1, const Jet& jet2, const Jet& bjet){
  ...
  //Input variables should be given in the same order as they appear in the top tagger!
  for (size_t i=0; i < inputNames.size(); i++){
    ...
    else std::cout<<"=== TopSelectionNN: Variable "<<var<<" is not ine of the input variables!"<<std::endl;
  }

  if (inputNames.size() > inputs.size()) std::cout<<"=== TopSelectionNN: Top tagger is expecting "<<inputNames.size()<<" variables. "<<inputs.size()<<" have been given!"<<std::endl;
  return inputs;
}
TrijetSelection TopSelectionNN::SortInMVAvalue(TrijetSelection TopCand){
  //  Description
...
SelectedTrijets TopSelectionNN::GetSelectedTopCandidate(TrijetSelection TopCand, int index){
...
bool TopSelectionNN::TopIsCrossCleaned(int Index, TrijetSelection TopCand){
  // Description:
...
vector<genParticle> TopSelectionNN::GetGenParticles(const vector<genParticle> genParticles, const int pdgId)
{
...
const genParticle TopSelectionNN::GetLastCopy(const vector<genParticle> genParticles, const genParticle &p){
...
vector<genParticle> TopSelectionNN::GetWpartons( genParticle daughter, const Event& event){
...
bool TopSelectionNN::IsGenuineTop(const Jet& trijetJet1, const Jet& trijetJet2, const Jet& trijetBJet,
